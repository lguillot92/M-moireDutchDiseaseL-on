...
egen exphuiles=sum(quantity) if product_sitc_fr=="Huiles, graisses et cires d'origine animale et végétale", by(customs_region year)
egen expbois=sum(quantity) if product_sitc_fr=="Produits du bois, liège, jonc", by(customs_region year)
egen reexp=sum(quantity) if product_reexportations=="Réexportation", by(customs_region year)
egen expgrain=sum(quantity) if product_grain != "Pas grain", by(customs_region year)
egen expcol=sum(quantity) if partner_grouping=="Amériques", by(customs_region year)
egen expind=sum(quantity) if product_threesectors=="Manufactures", by(customs_region year)
egen expmerdouille=sum(quantity) if product_sitc_fr=="Matières brutes non-comestibles non transformées n.d.a.", by(customs_region year)
...
foreach var of varlist expbois exphuiles reexp expgrain expcol expind expmerdouille {
bysort customs_region : gen fdl`var'lag=fdl`var'[_n-1] if year[_n-1]==year[_n]-1
}
foreach var of varlist expbois exphuiles reexp expgrain expcol expind expmerdouille {
bysort customs_region : gen fdl`var'lag2=fdl`var'[_n-2] if year[_n-2]==year[_n]-2
}
foreach var of varlist expbois exphuiles reexp expgrain expcol expind expmerdouille {
bysort customs_region : gen fdl`var'lag3=fdl`var'[_n-3] if year[_n-3]==year[_n]-3
}
...
foreach var of varlist expbois exphuiles reexp expgrain expcol expind expmerdouille {
bysort customs_region : gen l`var'lag2=log`var'[_n-2] if year[_n-2]==year[_n]-2
}
